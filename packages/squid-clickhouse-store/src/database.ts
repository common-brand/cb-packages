...
export type ClickhouseTableConfig = Record<
  string,
  { trackBy: string; relatedDicts?: string[] }
>;
...
export class ClickhouseDatabase {
  ...
  async transactHot2(
    ...
    if (chain[finalizedHeadPos].hash != info.finalizedHead.hash) {
      this.debug(
        '!!! chain[finalizedHeadPos].hash != info.finalizedHead.hash; %o',
        {
          finalizedHeadPos,
  ...
  private async deleteFinalizedHotBlocks(
    finalizedHeight: number
  ): Promise<void> {
    await this.client.exec(
      `DELETE
             FROM ${this.stateSchema}.hot_block ON CLUSTER cluster1
  ...
  private async insertHotBlock(block: HashAndHeight): Promise<void> {
    await this.client.insert(`${this.stateSchema}.hot_block`, [
  ...
  async getHotBlocks(): Promise<HashAndHeight[]> {
    const query = `SELECT hash, height
                       FROM ${this.stateSchema}.hot_block FINAL
                       WHERE chain_id = ${this.chainId}
                       ORDER BY height`;
    return await this.client.query<{ hash: string; height: number }>(query);
  }
